...
- `infra/terraform` — FleetMind AWS infrastructure module
...
|---|---|
| FleetMind CLI | `1.0.0` |
| `infra/terraform` | source baseline: standalone `v1.1.9`; no FleetMind release tag yet |
...
| FleetMind CLI | `infra/terraform` | `fleetmind-template` baseline | Compatibility notes |
|---|---|---|---|
...
- After the first FleetMind release containing `infra/terraform`, upgrade the template/module pin to `fleetmind//infra/terraform?ref=<FleetMind release tag>` before applying a newer FleetMind CLI to a fleet.
