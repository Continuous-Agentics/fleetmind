...
**Symptom:** You have one or more fleets on CLI workspaces (state under `env:/<workspace>/...` in the shared state bucket) and want to move to the recommended explicit-`key` pattern (see [Terraform module consumer setup](README.md#consumer-setup)) without downtime or state loss.
