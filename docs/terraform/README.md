...
Canonical Terraform implementation embedded in the FleetMind repository. Operators use `infra/terraform` as the Terraform root; the embedded module lives at `infra/terraform/modules/fleetmind` so existing `module.fleetmind.*` state addresses remain stable. FleetMind release tags are the supported consumer version contract.

## Layout

```
infra/terraform/modules/fleetmind/*.tf  # embedded module composition + cross-cutting locals
...
infra/terraform/modules/fleetmind/modules/agent/                      # one bot: EC2 + IAM role + per-agent secrets
infra/terraform/modules/fleetmind/modules/task-ledger/                # delegation substrate (DDB + S3 + IAM)
```
...
Start in `infra/terraform`. It is an operator root with an AWS provider, an
S3 backend stub, and `module "fleetmind" { source = "./modules/fleetmind" }`.
Configure `backend.hcl` from `backend.example.hcl`, then run:
...
The wrapper preserves deployed addresses including `module.fleetmind` and
`module.fleetmind.module.agent[...]`. Do not run the embedded module directly
against an existing fleet state.
...
- Per-agent EC2, IAM roles, and Secrets Manager placeholders (`infra/terraform/modules/fleetmind/modules/agent/`, one call per agent). Model-provider API keys live one secret **per (agent, provider)** at `<fleet_name>/agents/<agent>/providers/<provider>`; Slack + hooks secrets stay at `<fleet_name>/agents/<agent>/{slack,hooks}`.
...
- [`examples/terraform/basic`](../../examples/terraform/basic) — two-agent fleet, module-managed VPC, NATS, ContextStore, deploy-staging bucket, task-ledger.
- [`examples/terraform/existing-vpc`](../../examples/terraform/existing-vpc) — smallest BYO-VPC footprint.

## API reference

Generated by [`terraform-docs`](https://terraform-docs.io/) from the repository `.terraform-docs.yml`. Run `terraform-docs infra/terraform/modules/fleetmind` before a PR that changes variables, outputs, providers, modules, or resources.
...
|------|-------------|------|---------|:--------:|
| <a name="input_agent_github_apps"></a> [agent\_github\_apps](#input\_agent\_github\_apps) | Map of agent\_id → explicitly declared GitHub App names. IAM-only; credentials never enter Terraform state. | `map(list(string))` | `{}` | no |
...
| <a name="input_node_version"></a> [node\_version](#input\_node\_version) | Node.js major version to install on FleetMind agent instances through NodeSource RPMs. Only version 24 is supported. | `string` | `"24"` | no |
...
- [`EXISTING-VPC.md`](EXISTING-VPC.md) — BYO VPC mode, requirements, interface-endpoints limitation.
- [`TASK-LEDGER-STANDALONE.md`](TASK-LEDGER-STANDALONE.md) — calling `infra/terraform/modules/fleetmind/modules/task-ledger/` directly.
- [`MODULE-TROUBLESHOOTING.md`](MODULE-TROUBLESHOOTING.md) — state lock recovery, tfvars propagation, taint/replace, secret-deletion collisions.
- [`MIGRATIONS.md`](MIGRATIONS.md) — per-version upgrade notes.
