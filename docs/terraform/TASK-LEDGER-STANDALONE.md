...
The `modules/task-ledger/` submodule provisions the delegation substrate (DynamoDB table, S3 narratives bucket IAM access, and PM/worker/reader IAM policies). It's normally activated by the root `FleetMind Terraform module` module whenever `delegation_enabled = true` — that's the canonical path used by `fleetmind-template`.
...
module "task_ledger" {
  source = "github.com/Continuous-Agentics/FleetMind Terraform module//modules/task-ledger?ref=v1.1.9"
...
See [`infra/terraform/modules/task-ledger/variables.tf`](../../infra/terraform/modules/task-ledger/variables.tf) for the full input surface.
