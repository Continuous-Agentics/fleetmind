...
Intended to be invoked from the root `FleetMind Terraform module` module via `for_each` over the agent names. Cross-cutting policies (task-ledger PM/worker grants) are attached separately by the `task-ledger` submodule using `iam_role_name` from this module.
