...
module "agent" {
  ...
  github_apps     = lookup(var.agent_github_apps, each.key, [])
...
# -> SSM Run Command wake pipeline. That wake pipeline was removed; this
# submodule creates no wake infrastructure.
#
