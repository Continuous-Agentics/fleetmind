...
export function renderTerraformVars(fleet: Fleet): string {
  ...
  // Derive wake_target_session_key from the orchestrator agent's first Slack
  // channel. The TF task-ledger module uses this to configure the
  // DDB-to-OpenClaw wake rule (EventBridge/Lambda). Format:
  //   agent:main:slack:channel:<channel_id>
  // Omitted (with a comment) when no orchestrator or no Slack channels exist.
  const orchestrator = fleet.agents.list.find((a) => a.orchestrator);
  const firstChannel = orchestrator?.slack?.channels?.[0];
  ...
  if (wakeSessionKey) {
    lines.push(
      `# OpenClaw session key for the PM bot — used by the DDB-to-OpenClaw wake`,
      `# rule (EventBridge/Lambda) to target the correct session on task ledger`,
      `# changes. Derived from the orchestrator agent's first Slack channel.`,
      `wake_target_session_key = "${wakeSessionKey}"`,
      ``,
    );
  } else {
    lines.push(
      `# wake_target_session_key: not emitted — no orchestrator agent with a`,
      `# Slack channel found. Set this manually in your workspace tfvars.`,
      ``,
    );
  }
