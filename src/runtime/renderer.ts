...
export function renderTerraformVars(fleet: Fleet): string {
  ...
  if (wakeSessionKey) {
    lines.push(
      ...
      `# rule (EventBridge Pipe → SSM/hooks) to target the correct session on`,
      `# task-ledger changes. Derived from the orchestrator's first Slack channel.`,
    ...
    lines.push(
      ...
      `# Slack channel found. Set this manually in your workspace tfvars if`,
      `# delegation is enabled, or the wake rule will not be wired up.`,
      ``,
    );
  }
