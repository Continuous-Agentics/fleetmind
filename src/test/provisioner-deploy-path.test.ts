...
describe("renderTerraformVars", () => {
  function makeMinimalFleet(): Fleet {
    return {
      ...makeFleet(),
      agents: {
        defaults: {
  ...
  test("emits fleet_name, agent_names, agent_orchestrators", () => {
    const fleet = makeMinimalFleet();
    fleet.agents.list = [makeConductorAgent(), makeForgeAgent()];
    const out = renderTerraformVars(fleet);
    assert.ok(out.includes(`fleet_name  = "gg-sandbox"`), "must include fleet_name");
  ...
  test("emits wake_target_session_key from orchestrator first Slack channel", () => {
    const fleet = makeMinimalFleet();
    const conductor: AgentConfig = {
      ...makeConductorAgent(),
      slack: {
        account_id: "conductor",
        bot_token: "xoxb-test",
        app_token: "xapp-test",
        channels: ["C07GGJPQJCD", "C0B5ADA7REH"],
        extra_scopes: [],
        extra_events: [],
      } as AgentConfig["slack"],
    };
    fleet.agents.list = [conductor, makeForgeAgent()];
    const out = renderTerraformVars(fleet);
    assert.ok(
    ...
    assert.ok(!out.includes("C0B5ADA7REH"), "must NOT use second channel");
  });
  ...
  test("omits wake_target_session_key when no orchestrator agent exists", () => {
    const fleet = makeMinimalFleet();
    fleet.agents.list = [makeForgeAgent()]; // no orchestrator
    const out = renderTerraformVars(fleet);
    assert.ok(!out.includes("wake_target_session_key ="), "must not emit var when no orchestrator");
  });

  test("omits wake_target_session_key when orchestrator has no Slack config at all", () => {
    const fleet = makeMinimalFleet();
    const conductor = { ...makeConductorAgent(), slack: undefined } as unknown as AgentConfig;
    fleet.agents.list = [conductor];
    const out = renderTerraformVars(fleet);
    assert.ok(!out.includes("wake_target_session_key ="), "must not emit var when slack is undefined");
  });

  test("output is auto-generated header comment", () => {
    const fleet = makeMinimalFleet();
    fleet.agents.list = [makeConductorAgent()];
    const out = renderTerraformVars(fleet);
    assert.ok(out.startsWith("# Auto-generated by FleetMind"), "must start with auto-gen comment");
    assert.ok(out.includes(`# Fleet: gg-sandbox`), "must include fleet name in header");
  });
});
