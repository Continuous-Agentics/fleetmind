...
import { renderOpenClawJson, renderAgentOpenClawJson, writeOutputs, renderTerraformVars } from "../runtime/renderer.js";
...
// ── renderTerraformVars — wake_target_session_key derivation ───────────────────

describe("renderTerraformVars — wake_target_session_key", () => {
  /** Build a conductor (orchestrator) whose slack channel entry has the given channel IDs. */
  function conductorWithChannels(channels: string[]): AgentConfig {
    const c = makeConductorAgent() as unknown as { channels: Array<Record<string, unknown>> };
    c.channels = [{
      provider: "slack",
      account_id: "conductor",
      bot_token: "xoxb-test",
  ...
  test("emits fleet_name, agent_names, agent_orchestrators", () => {
    const fleet = makeFleet();
    fleet.agents.list = [makeConductorAgent(), makeForgeAgent()];
    const out = renderTerraformVars(fleet);
    assert.ok(out.includes(`fleet_name  = "dogfood"`), "must include fleet_name");
    assert.ok(out.includes(`agent_names = ["conductor", "forge"]`), "must include agent_names");
    assert.ok(out.includes("conductor = true"), "orchestrator entry must be true for conductor");
    assert.ok(out.includes("forge = false"), "orchestrator entry must be false for forge");
  });

  test("emits wake_target_session_key from orchestrator's first Slack channel", () => {
    const fleet = makeFleet();
  ...
  test("omits wake_target_session_key when orchestrator has no Slack channels", () => {
    const fleet = makeFleet();
    fleet.agents.list = [conductorWithChannels([]), makeForgeAgent()];
    const out = renderTerraformVars(fleet);
    assert.ok(!out.includes("wake_target_session_key ="), "must not emit the var when no channel");
    assert.ok(out.includes("not emitted"), "must include the explanatory comment");
  });

  test("omits wake_target_session_key when no orchestrator agent exists", () => {
    const fleet = makeFleet();
    fleet.agents.list = [makeForgeAgent()]; // forge is not an orchestrator
    const out = renderTerraformVars(fleet);
    assert.ok(!out.includes("wake_target_session_key ="), "must not emit var when no orchestrator");
  });

  test("omits wake_target_session_key when orchestrator has no Slack channel entry at all", () => {
    const fleet = makeFleet();
    const c = makeConductorAgent() as unknown as { channels: unknown[] };
    c.channels = []; // no slack entry
    fleet.agents.list = [c as unknown as AgentConfig];
    const out = renderTerraformVars(fleet);
    assert.ok(!out.includes("wake_target_session_key ="), "must not emit var when slack channel absent");
  });
  ...
  test("output starts with the auto-generated header", () => {
    ...
    fleet.agents.list = [makeConductorAgent()];
    const out = renderTerraformVars(fleet);
    assert.ok(out.startsWith("# Auto-generated by FleetMind"), "must start with auto-gen comment");
    assert.ok(out.includes(`# Fleet: dogfood`), "must include fleet name in header");
  });
});
